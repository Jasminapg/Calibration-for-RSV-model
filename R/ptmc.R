...
ptmc_func <- function(model, data, settings) {
  ...
  outPTacc <- vector(mode = "list", length = settings[["nrChains"]])
  ...
  for (i in 1:settings[["nrChains"]]){
    ...
    outPTacc[[i]] <- out_raw[, settings$P+3]
  }
  ...
  colnames(outlaccv) <- c(1:settings[["nrChains"]])
  outlaccv <- outlaccv %>% gather(colnames(outlaccv), key="chain_no", value="acceptance rate")
  outlaccv$sample_no <- rep(1:length(outPTacc[[1]]), settings[["nrChains"]])
